...
def _build_prompt(location: str, duration_minutes: int, style: str, source_text: str = "") -> str:
    target_words = duration_minutes * WORDS_PER_MINUTE
    style_line = STYLE_INSTRUCTIONS.get(style, STYLE_INSTRUCTIONS["historical"])
    source_text = source_text.strip()

    # עם טקסט מקור (ויקיפדיה): עיגון קשיח - בלי זה, מקומות קטנים/פחות
    # מוכרים (כמו יישובים קהילתיים) גורמים ל-Gemini "להשלים" בביטחון מלא
    # תאריכי ייסוד, ציטוטים וארכיטקטורה שאף פעם לא היו - כי אין לו עליהם
    # ידע אמיתי אבל הוא לא מגלה את זה בפלט.
    if source_text:
        grounding_rule = (
            "2. הסתמך אך ורק על טקסט המקור המצורף בסוף. אסור להמציא תאריכים, שמות, "
            "ציטוטים, מבנים או אירועים שאינם מופיעים בו. אם הטקסט קצר או חסר פרטים "
            "לאורך הנדרש - הרחב בתיאור כללי (נוף, אופי המקום, הקשר גיאוגרפי/תרבותי "
            "רחב) במקום להמציא פרטים ספציפיים."
        )
    else:
        grounding_rule = (
            "2. אין ברשותך טקסט מקור למקום זה. הסתמך רק על עובדות שאתה בטוח בהן "
            "ברמת ודאות גבוהה מאוד. אסור בהחלט להמציא תאריכים מדויקים, ציטוטים, "
            "שמות מבנים או פרטים ארכיטקטוניים ספציפיים שאינך בטוח בהם - אם אינך "
            "בטוח, העדף תיאור כללי (מיקום, אופי, הקשר היסטורי-תרבותי רחב וידוע) "
            "על פני פרט מומצא שנשמע משכנע."
        )
    ...
    parts = [
        ...
        "10. כתוב מספרים, שנים ותאריכים במילים ולא בספרות (למשל 'שנת אלף תשע",
        "    מאות שבעים ושתיים', לא '1972') - מנוע ההקראה קורא מילים בצורה טבעית",
        "    הרבה יותר מרצף ספרות.",
    ]
    if source_text:
        parts += ["", "--- טקסט המקור (ויקיפדיה) ---", source_text, "--- סוף טקסט המקור ---"]
    parts += ["", "כתוב כעת רק את תסריט הסיור עצמו:"]
    return "\n".join(parts)
...
async def generate_script(location: str, duration_minutes: int, style: str, source_text: str = "") -> str:
    if not GEMINI_API_KEY:
        raise RuntimeError("GEMINI_API_KEY חסר. הוסף אותו ל-backend/.env")

    # תקרה על אורך המקור - שולטת בעלות בלי לפגוע בעיגון (ערכי ויקיפדיה
    # ארוכים באמת נדירים; זה בעיקר רשת ביטחון).
    prompt = _build_prompt(location, duration_minutes, style, source_text[:8000])
...
async def generate_highlights(location: str, source_text: str = "") -> list[dict]:
    ...
    if not GEMINI_API_KEY or not location.strip():
        return []
    source_text = source_text.strip()[:8000]
    grounding = (
        "2. הסתמך אך ורק על טקסט המקור המצורף בסוף. אסור להמציא תאריכים, שמות או פרטים שאינם בו."
        if source_text
        else "2. הסתמך רק על ידע עובדתי אמין ברמת ודאות גבוהה. אסור להמציא תאריכים או פרטים ספציפיים."
    )
    prompt_lines = [
    ...
    if source_text:
        prompt_lines += ["", "--- טקסט המקור (ויקיפדיה) ---", source_text, "--- סוף טקסט המקור ---"]
    prompt = "\n".join(prompt_lines)
    payload = {
        "contents": [{"parts": [{"text": prompt}]}],
        "generationConfig": {
