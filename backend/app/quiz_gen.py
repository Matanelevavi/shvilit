...
async def generate_quiz(location: str, count: int = 5, source_text: str = "") -> List[dict]:
    if not GEMINI_API_KEY:
        raise RuntimeError("GEMINI_API_KEY חסר.")

    source_text = source_text.strip()[:8000]
    # בלי עיגון, "תאריכים היסטוריים, ממדים, אירועים ספציפיים" מהחוק ה-3
    # למטה זו בדיוק המתכון להמצאת "תשובה נכונה" שגויה - חמור בחידון יותר
    # מאשר בתסריט, כי המשתמש לומד את הפרט המומצא כעובדה.
    grounding = (
        "6. הסתמך אך ורק על טקסט המקור המצורף בסוף. אסור לשאול על תאריך, מספר או "
        "פרט שאינו מופיע בו במפורש - התשובה ה'נכונה' חייבת להיות מאומתת בטקסט."
        if source_text
        else "6. הסתמך רק על עובדות שאתה בטוח בהן ברמת ודאות גבוהה מאוד. אם אינך "
        "בטוח בתאריך/מספר מדויק - אל תשאל עליו, גם אם זה מצמצם את מגוון השאלות."
    )
